...
# device = torch.device("cuda:1")  # For the first GPU
# torch.cuda.set_device(1)  # 将设备设置为第二张显卡，编号为1
RANDOM_SEED = 100
...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'PhysFormer_small':
        model_trainer = trainer.PhysFormer_small_Trainer.PhysFormer_small_Trainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'SpikingPhysFormer_v3':
        model_trainer = trainer.SpikingPhysFormerTrainer_v3.SpikingPhysFormerTrainer_v3(config,data_loader_dict)
    else:
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        if config.TRAIN.DATA.DATASET == "UBFC-rPPG":
            ...
            train_loader = data_loader.vv100Loader.vv100Loader
        elif config.TRAIN.DATA.DATASET == "vv250":
            train_loader = data_loader.vv250Loader.vv250Loader
        elif config.TRAIN.DATA.DATASET == "vvAll":
            train_loader = data_loader.vvAllLoader.vvAllLoader
        else:
        ...
        if config.VALID.DATA.DATASET == "UBFC-rPPG":
            ...
            valid_loader = data_loader.vv100Loader.vv100Loader
        elif config.VALID.DATA.DATASET == "vv250":
            valid_loader = data_loader.vv250Loader.vv250Loader
        elif config.VALID.DATA.DATASET == "vvAll":
            valid_loader = data_loader.vvAllLoader.vvAllLoader
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        elif config.TEST.DATA.DATASET == "vv250":
            test_loader = data_loader.vv250Loader.vv250Loader
        elif config.TEST.DATA.DATASET == "vvAll":
            test_loader = data_loader.vvAllLoader.vvAllLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), and UBFC-PHYS.")
        ...
        elif config.UNSUPERVISED.DATA.DATASET == "vv100":
            unsupervised_loader = data_loader.vv100Loader.vv100Loader
        elif config.UNSUPERVISED.DATA.DATASET == "vv250":
            unsupervised_loader = data_loader.vv250Loader.vv250Loader
        elif config.UNSUPERVISED.DATA.DATASET == "vvAll":
            unsupervised_loader = data_loader.vvAllLoader.vvAllLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
