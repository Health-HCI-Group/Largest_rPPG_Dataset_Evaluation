...
class BaseLoader(Dataset):
    ...
    def augment_data(video_frames, bvp_data, sr, shape, extend_rate=1, extend_hr=(40, 150), step=1):

        def resize(frames, wave, length):
            """
            Scale frames and waves in time to length frames.
            """
            # Reshape frames to a 2D array
            frames_reshaped = frames.reshape(frames.shape[0], -1, frames.shape[-1])

            # Resize frames in the time dimension
            resized_frames = cv2.resize(frames_reshaped, (frames_reshaped.shape[1], length), interpolation=cv2.INTER_NEAREST)

            # Reshape frames back to the original shape
            resized_frames = resized_frames.reshape(length, *frames.shape[1:])

            # Interpolate waves to match the resized frames in time
            interpolated_waves = [UnivariateSpline(np.linspace(0, 1, wave.shape[0]), wave, s=0)(np.linspace(0, 1, length))]

            return resized_frames, interpolated_waves
        
        n = 0
        frames_augs = list()
        bvps_augs = list()
        while True:
            frames = video_frames[n:n+shape]
            if frames.shape < shape:
                break

            frames_augs.append(frames)
            bvps_augs.append(bvp_data[n:n+shape[0]])
            ...
            while extend_rate > 0 and np.random.rand() < 1 / (1 + 1 / extend_rate):
                ...
                hr = bvps_augs
                frames_target = max(round(shape * hr_target / hr), 4)  # Minimum 4 frames

                if n + frames_target > video_frames.shape:
                    break

                resized_frames, resized_bvp_data = resize(video_frames[n:n+frames_target], [bvp_data[n:n+frames_target]], shape)

                frames_augs.append(resized_frames)
                bvps_augs.append(resized_bvp_data)

            n += round(sr * step)
            
            frames_augs = np.asarray(frames_augs)
            bvps_augs = np.asarray(bvps_augs)

        return frames_augs, bvps_augs


    def preprocess(self, frames, bvps, config_preprocess, ext_fps = 0):
        ...
        # if(ext_fps > 0):
        #     frames_augs, bvps_augs = self.augment_data(data, bvps, ext_fps, config_preprocess.CHUNK_LENGTH)
        #     frames_clips = np.concatenate((frames_clips,frames_augs), axis=0)
        #     bvps_clips = np.concatenate((bvps_clips,bvps_augs), axis=0)
