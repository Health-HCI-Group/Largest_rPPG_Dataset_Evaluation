...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    ...
    def __init__(
        ...
        if self.num_layers != 0:
            self.transformer = Transformer_ST_TDC_gra_sharp(num_layers=num_layers, dim=dim, num_heads=num_heads, 
    ...
    def forward(self, x, gra_sharp):
        ...
        if self.num_layers != 0:
            Trans_features, Score1 =  self.transformer(x, gra_sharp)  # [B, 4*4*40, 64]
        else:
            Trans_features, Score1 = x, gra_sharp
        ...
        rPPG = rPPG.squeeze(1)
        return rPPG, Score1, Score1, Score1
