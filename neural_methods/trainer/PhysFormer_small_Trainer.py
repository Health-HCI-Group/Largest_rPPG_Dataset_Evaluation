...
from neural_methods.model.PhysFormer_small import ViT_ST_ST_Compact3_TDC_gra_sharp
...
class PhysFormer_small_Trainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            """
            print(self.model)
            input = torch.randn(1, 3, 160, 128, 128).to(self.device) # 输入数据，尺寸根据你的模型实际情况来定
            gra_sharp = torch.randn(1).to(self.device)
            flops, params = profile(self.model, inputs=(input, gra_sharp))
            print(f"FLOPs: {flops/160}")
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                if(self.config.TRAIN.DATA.DATASET == "MMPD"):
                    batch[0] = batch[0].permute(0,2,1,3,4)  # MMPD
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                if(self.config.TEST.DATA.DATASET == "MMPD"):
                    test_batch[0] = test_batch[0].permute(0,2,1,3,4)  # MMPD
